...
export async function fetchHealth(): Promise<HealthResponse> {
  if (isStaticDemo()) return staticApi.fetchHealth()
  return request('/api/health')
}

export async function register(username: string, password: string) {
  if (isStaticDemo()) return staticApi.register(username, password)
...
export async function login(username: string, password: string) {
  if (isStaticDemo()) return staticApi.login(username, password)
...
export async function fetchMe(): Promise<User> {
  if (isStaticDemo()) return staticApi.fetchMe()
  return request('/api/auth/me')
}

export async function listProjects(): Promise<Project[]> {
  if (isStaticDemo()) return staticApi.listProjects()
  return request('/api/projects')
}

export async function createProject(prompt: string, title?: string, model?: string): Promise<Project> {
  if (isStaticDemo()) return staticApi.createProject(prompt, title)
  return request('/api/projects', {
    method: 'POST',
...
export async function getProject(pid: number) {
  if (isStaticDemo()) return staticApi.getProject(pid)
  return request<
    Project & {
...
export async function deleteProject(pid: number): Promise<void> {
  if (isStaticDemo()) return staticApi.deleteProject(pid)
...
export async function postMessage(pid: number, content: string, model?: string): Promise<Project> {
  if (isStaticDemo()) return staticApi.postMessage(pid, content)
...
export async function stopProject(pid: number): Promise<void> {
  if (isStaticDemo()) return staticApi.stopProject(pid)
  await request(`/api/projects/${pid}/stop`, { method: 'POST' })
}

export async function listVersions(pid: number) {
  if (isStaticDemo()) return staticApi.listVersions(pid)
...
export async function getVersionFiles(pid: number, version: number) {
  if (isStaticDemo()) return staticApi.getVersionFiles(pid, version)
...
export async function saveFile(pid: number, path: string, content: string) {
  if (isStaticDemo()) return staticApi.saveFile(pid, path, content)
...
export async function rollbackVersion(pid: number, version: number) {
  if (isStaticDemo()) return staticApi.rollbackVersion(pid, version)
...
export async function publishProject(pid: number) {
  if (isStaticDemo()) return staticApi.publishProject(pid)
...
export async function reportFix(pid: number, message: string, source: string) {
  if (isStaticDemo()) return staticApi.reportFix(pid, message, source)
